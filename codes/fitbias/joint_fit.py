...
def chi2(p):
    ...
    # Evaluate the theory.
    if len(p)==7:
        b1,b2,bs,alpha1,sn1,alpha2,sn2=p[0],p[1],p[2],p[3],p[4],p[5],p[6]
        bn=0
        sn1=0
    else:
...
def fit_example(verbose=False, lo = False):
    ...
    pname = ["b1","b2","bs","alpha1","sn1","alpha2","sn2"]
    fname = "./log_jointfit/joint_fit_results.log"
    if lo:
        fname = "./log_jointfit/joint_fit_results_lo.log"
    ff= open(fname,"w")
